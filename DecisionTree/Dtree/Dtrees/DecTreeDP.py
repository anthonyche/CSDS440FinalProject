...
class DecisionTree(object):
    ...
    def _build_tree(self,X,y,current_depth=0):
        ...
        if largest_impurity > self.min_impurity:
            # As long as it is not pure node
...
class ClassificationTree(DecisionTree):
    ...
    def _majority_vote(self, y):
        most_common = None #initialize
        max_count = 0 #initialize

        #check different value in y:
        #count the number of differnt y
        #calculate posibility
        #add noise to the posibility
        #two norm
        label_count = []
...
def main():
    ...
    sensitivity = max(norm_list) - min(norm_list)
    X += np.random.laplace(0, sensitivity/eps, size=X.shape)
